...
export const chatService = {
  ...
  async flagChat(chatId: string): Promise<boolean> {
    const { error } = await supabase
      .from('chats')
      .update({
        is_flagged: true,
  ...
  async unflagChat(chatId: string): Promise<boolean> {
    const { error } = await supabase
      .from('chats')
      .update({
        is_flagged: false,
  ...
  async muteChat(chatId: string): Promise<boolean> {
    const { error } = await supabase
      .from('chats')
      .update({
        is_muted: true,
  ...
  async unmuteChat(chatId: string): Promise<boolean> {
    const { error } = await supabase
      .from('chats')
      .update({
        is_muted: false,
  ...
  async deleteChat(chatId: string): Promise<boolean> {
    // Delete all messages first (soft delete)
    await supabase
      .from('messages')
      .update({
        deleted_at: new Date().toISOString(),
