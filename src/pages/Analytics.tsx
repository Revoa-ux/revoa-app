...
import { RefreshCw, Edit3, X, Plus, Info, AlertCircle, WifiOff, ArrowRight, Store } from 'lucide-react';
          ...
          {(() => {
            ...
            if (shopify.isConnected) {
              connectedPlatformsList.push({
                id: 'shopify',
                name: 'Shopify',
                icon: <Store className="w-3.5 h-3.5 text-[#96BF48]" />
              });
            } else {
                          ...
                          {unconnectedPlatformsList.map((platform, index) => (
                            ...
                            >
                              <div className="flex items-center justify-center w-8 h-8 bg-gray-100 dark:bg-gray-700 rounded-lg flex-shrink-0">
                                {platform.id === 'shopify' && <Store className="w-5 h-5 text-gray-900 dark:text-white" />}
