...
async function main() {
  ...
  if (storeError || !store) {
    console.error('❌ Error: Could not find revoatest store');
    console.error('   Make sure the store exists in the database');
    process.exit(1);
  }

  console.log(`✅ Found store: ${store.store_url}`);
  ...
  if (products.length === 0) {
    console.error('❌ Error: No products found in store');
  ...
  if (!firstVariant) {
    console.error('❌ Error: Product has no variants');
    process.exit(1);
  }

  console.log(`✅ Using product: ${firstProduct.title}`);
  ...
  for (let i = 0; i < NUM_ORDERS; i++) {
    try {
      ...
      if (orderNum % 10 === 0) {
        console.log(`✅ Created ${orderNum}/${NUM_ORDERS} orders (${successCount} successful, ${failureCount} failed)`);
      }
  ...
  if (failureCount > 0) {
    console.log('⚠️  Some orders failed. Check the errors above.');
    process.exit(1);
  }
