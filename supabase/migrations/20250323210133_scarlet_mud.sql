/*
  # Fix User Creation and Email Uniqueness

  1. Changes
    - Drop existing triggers
    - Create new user creation trigger with proper conflict handling
    - Update email uniqueness constraints
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.user_profiles (
      ...
      false,
      CASE 
        WHEN NEW.email LIKE '%@revoa.app' THEN true
        ELSE false
      END,
      CASE 
        WHEN NEW.email = 'tyler.jtw@gmail.com' THEN 'super_admin'
        WHEN NEW.email LIKE '%@revoa.app' THEN 'admin'
        ELSE NULL
      END,
      CASE
        WHEN NEW.email = 'tyler.jtw@gmail.com' THEN ARRAY['manage_users', 'manage_admins', 'manage_settings']
        WHEN NEW.email LIKE '%@revoa.app' THEN ARRAY['manage_users', 'manage_settings']
        ELSE NULL
      END,
      CASE
        WHEN NEW.email LIKE '%@revoa.app' OR NEW.email = 'tyler.jtw@gmail.com' THEN 'active'
        ELSE NULL
      END,
      jsonb_build_object(
